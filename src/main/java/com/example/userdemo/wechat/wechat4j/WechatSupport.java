package com.example.userdemo.wechat.wechat4j;

import com.example.userdemo.wechat.util.XmlUtil;
...
import org.sword.wechat4j.response.*;
...
public abstract class WechatSupport {
    ...
    public abstract void InitRequest(HttpServletRequest request);
    ...
    public String execute() {
        ...
        logger.info("Wechat xml data:" + result);
    ...
    private String dispatch() {
        ...
        try {
            ...
            postDataStr = XmlUtil.convertStreamToString(inputStream);
    ...
    private void setPostData(String xmlStr) {
        ...
        try {
            JAXBContext context = JAXBContext.newInstance(WechatRequest.class);
            Unmarshaller unmarshaller = context.createUnmarshaller();
            this.wechatRequest = (WechatRequest) unmarshaller.unmarshal(new StringReader(xmlStr));
        } catch (Exception e) {
     ...
     * @param content 回复的消息内容（换行：在content中能够换行，微信客户端就支持换行显示）
     */
    public void responseKeFuText(String content) {
        responseBase();
        wechatResponse.setMsgType("transfer_customer_service");
        wechatResponse.setContent(content);
    }
